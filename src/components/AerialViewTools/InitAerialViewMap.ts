...
export function initAerialView(divId:string) {
   // Token
  Cesium.Ion.defaultAccessToken = import.meta.env.VITE_AERIALVIEW_TOKEN

  window.aerialView = new Cesium.Viewer(divId, {
  ...
  (window as any).aerialView.cesiumWidget.creditContainer.style.display = "none"; // * 去除版权信息

  window.aerialView.scene.screenSpaceCameraController.zoomEventTypes = [Cesium.CameraEventType.WHEEL, Cesium.CameraEventType.PINCH];
  window.aerialView.scene.screenSpaceCameraController.tiltEventTypes = [Cesium.CameraEventType.PINCH, Cesium.CameraEventType.LEFT_DRAG];
  window.aerialView.scene.screenSpaceCameraController.enableRotate = false


  // // 设置鹰眼图中球属性
  // const control = window.aerialView.scene.screenSpaceCameraController
