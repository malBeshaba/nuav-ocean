...
// 获取飞行控制权
export async function postFlightAuth (sn: string) {
...
// 飞向目标点
export async function postFlyToPoint (sn: string, body: PostFlyToPointBody) {
  const resp = await request.post(`${API_PREFIX}/devices/${sn}/jobs/fly-to-point`, body)
  return resp.data
}

// 停止飞向目标点
export async function deleteFlyToPoint (sn: string) {
...
// 一键起飞
export async function postTakeoffToPoint (sn: string, body: PostTakeoffToPointBody) {
  const resp = await request.post(`${API_PREFIX}/devices/${sn}/jobs/takeoff-to-point`, body)
  return resp.data
}

export async function retuenHome (sn: string) {
